package de.geheimagentnr1.manyideascore.elements.commands.givedb;
...
public class GiveDBCommand {
	...
	public static void register( CommandDispatcher<CommandSource> dispatcher ) {
		...
		LiteralArgumentBuilder<CommandSource> givedbCommand = Commands.literal( "givedb" ).requires(
			commandSource -> commandSource.hasPermissionLevel( 2 )
		);
		givedbCommand.executes( command -> {
			command.getSource().sendFeedback(
				new StringTextComponent( "/givedb <target> <name of a dye block> <color>" ), true );
			return 1;
		} );
		givedbCommand.then( Commands.argument( "targets", EntityArgument.players() )
			.then( Commands.argument( "dye_item", DyeItemArgument.dyeItem() )
				.then( Commands.argument( "color", ColorArgument.color() )
					.executes( context ->
						giveItem( context.getSource(), DyeItemArgument.getItem( context, "dye_item" ),
	...
	private static int giveItem( CommandSource source, Item item, Color color,
		...
		if( targets.size() == 1 ) {
			source.sendFeedback( new TranslationTextComponent( "commands.give.success.single", count,
